...
function addToTable() {
    ...
    let btnCode = "<button class='remove-btn' onclick='removeToTable(this)'><strong>Deletar</strong></button>";
...
function removeToTable(id){
    ...
    let confirmacao = confirm("Deseja deletar realmente?")
        if (confirmacao==true)
        {
            confirmacao="você excluiu com sucesso!";
            row.parentNode.removeChild(row); //Removendo a linha
        }
        else
        {
            alert('Cadastro não excluido!')
            confirmacao="cadastro não excluido!";
        }
